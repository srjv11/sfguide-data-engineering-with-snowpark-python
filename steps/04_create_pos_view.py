...
def create_pos_view(session: Session):
    ...
    order_detail: Table = session.table("RAW_POS.ORDER_DETAIL").select(
    ...
    order_header: Table = session.table("RAW_POS.ORDER_HEADER").select(
        F.col("ORDER_ID"),
        F.col("TRUCK_ID"),
        F.col("ORDER_TS"),
    ...
    truck: Table = session.table("RAW_POS.TRUCK").select(
        F.col("TRUCK_ID"),
        F.col("PRIMARY_CITY"),
        F.col("REGION"),
        F.col("COUNTRY"),
    ...
    menu: Table = session.table("RAW_POS.MENU").select(
        F.col("MENU_ITEM_ID"),
        F.col("TRUCK_BRAND_NAME"),
        F.col("MENU_TYPE"),
        F.col("MENU_ITEM_NAME"),
    )
    franchise: Table = session.table("RAW_POS.FRANCHISE").select(
        F.col("FRANCHISE_ID"),
        F.col("FIRST_NAME").alias("FRANCHISEE_FIRST_NAME"),
        F.col("LAST_NAME").alias("FRANCHISEE_LAST_NAME"),
    )
    location: Table = session.table("RAW_POS.LOCATION").select(F.col("LOCATION_ID"))

    """
    We can do this one of two ways:
        either select before the join so it is more explicit, or
        just join on the full tables.
...
if __name__ == "__main__":
    ...
    session: Session = snowpark_utils.get_snowpark_session()
